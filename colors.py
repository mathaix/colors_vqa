...
class ColorsCorpusExample:
    ...
    def create_image(self, index, image_id, rgb, position, border,  distractor_index=None):

        borders = ["red", "green", "yellow"]
        rand_border = 4

        question = "Describe the selected color"
        question_extension = " bounded by the %s border" 

        target_index = position.index(self._get_target_index('speaker'))
        ...
        for i, c in enumerate(rgb):
                ...
                if i == target_index:
                    ...
                    if index == rand_border: # randomly set a different border for non target colors                    
                        if (i == distractor_index): 
                            ec = borders[random.randint(0, 2)]
                            quest = question  + question_extension % ("black")
        ...
        file_name = "image_%s.png" % (image_id)
        file_name = os.path.join(self.img_dir, file_name)
        plt.savefig(file_name)
        plt.close()

        data = {
            "id"   : "%s_%s" % (self.gameid ,self.roundNum),
            "image_id"  : image_id,
            "question"  : quest,
    ...
    def generate_data(self, csvwriter, datadir, shape='rect', border="black" ):
        import itertools
        import random
        ...
        positions =  (list(itertools.permutations([0,1,2])))
        rgb_premutations = list(itertools.permutations(rgbs))

        for z, rgb in enumerate(rgb_premutations):
            random_pos = random.randint(1, 2)
            image_id = "%s_%s_%s" % (self.gameid, self.roundNum, z)
            self.create_image(index = z, image_id = image_id, rgb = rgb, position = positions[z], 
                        border =border,  distractor_index=random_pos)
